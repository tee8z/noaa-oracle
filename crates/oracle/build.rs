...
/// Minifies each script on its own and joins them. Each is parsed as a
/// classic script, so its top-level names stay global and are not renamed.
/// A script the parser rejects fails the build.
