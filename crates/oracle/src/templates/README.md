...
2. Collects every `.js` file and minifies them with oxc into one classic
   script, so top-level names are shared across files. A script that does
   not parse fails the build.
