...
pub fn base(config: &PageConfig, content: Markup) -> Markup {
    html! {
        ...
        html lang="en" {
            ...
            body {
                script {
                    (PreEscaped(format!("const API_BASE = {};", script_string(config.api_base))))
                }
...
/// A JSON string literal safe inside a `<script>` element: JSON escaping
/// handles quotes and control characters, and `<` is escaped so the value
...
mod tests {
    ...
    #[test]
    fn script_strings_cannot_break_out_of_the_script() {
        assert_eq!(script_string("https://x.test"), r#""https://x.test""#);
        let hostile = script_string("\";alert(1);//</script><script>");
        assert!(!hostile.contains("</script>"));
        assert!(hostile.starts_with('"') && hostile.ends_with('"'));
        assert_eq!(
            serde_json::from_str::<String>(&hostile.replace("\\u003c", "<")).unwrap(),
