use maud::{DOCTYPE, Markup, PreEscaped, html};

use crate::templates::{
    assets,
    components::{tabs, theme_toggle},
};

pub struct PageConfig<'a> {
    pub title: &'a str,
    pub current_page: CurrentPage,
}

#[derive(Clone, Copy, PartialEq, Eq)]
pub enum CurrentPage {
    Dashboard,
    Events,
    RawData,
}

pub fn base(config: &PageConfig, content: Markup) -> Markup {
    html! {
        (DOCTYPE)
        html lang="en" {
            head {
                meta charset="UTF-8";
                meta name="viewport" content="width=device-width, initial-scale=1.0";
                title { (config.title) }
                link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bulma@1.0.4/css/bulma.min.css";
                link rel="stylesheet" href=(assets::CSS_URL);
                script defer src="https://cdn.jsdelivr.net/npm/htmx.org@1.9.10/dist/htmx.min.js" {}
                script defer src=(assets::JS_URL) {}
                // Apply the saved theme before the page paints.
                script { (PreEscaped(THEME_INIT_SCRIPT)) }
            }
            body {
                div class="container site" {
                    header class="site-header" {
                        a href="/" class="site-title" { "4cast Truth Oracle" }
                        div class="site-actions" {
                            (theme_toggle())
                            a href="/docs" class="button is-small" { "API docs" }
                            a href="https://github.com/tee8z/noaa-oracle" class="site-github"
                              aria-label="Source on GitHub" {
                                (github_icon())
                            }
                        }
                    }
                    (tabs(config.current_page, false))
                    main id="main-content" {
                        (content)
                    }
                }
            }
        }
    }
}

/// What htmx swaps into `#main-content` when a tab or link opens a page:
/// the page's content, its title, and the tabs out of band.
pub fn page_fragment(config: &PageConfig, content: Markup) -> Markup {
    html! {
        title { (config.title) }
        (tabs(config.current_page, true))
        (content)
    }
}

/// Sets `data-theme` from the saved choice, else the system preference.
const THEME_INIT_SCRIPT: &str = "document.documentElement.setAttribute('data-theme',\
localStorage.getItem('theme')||(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'));";

fn github_icon() -> Markup {
    html! {
        svg height="24" width="24" viewBox="0 0 16 16" version="1.1" aria-hidden="true"
            fill="currentColor" {
            path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z" {}
        }
    }
}
