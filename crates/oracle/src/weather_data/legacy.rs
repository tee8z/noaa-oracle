//! The forecast query of oracle 2.2.0, frozen as the reference the current
//! query must agree with. Its issue window and publication grace are copied
//! here too, so changing them in the current query shows up as a mismatch.

use super::{
    Error, Forecast, WeatherAccess, decode_forecasts, sql_string_list, station_filter,
    utc_timestamp_sql,
};
use crate::{file_access::FileParams, routes::ForecastRequest};
...
pub(super) async fn forecasts_data(
    ...
    if matches!((file_params.start, file_params.end), (Some(start), Some(end)) if start > end) {
        return Ok(vec![]);
    }
